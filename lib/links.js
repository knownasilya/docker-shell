...
function filterContainers(docker, label, value) {
  return new Bluebird((resolve, reject) => {
    ...
    docker.listContainers(opts, (err, res) => {
      if (err) {
        return reject(err);
      }
...
function resolveLinks(docker, links) {
  return Bluebird.map(links, (link) => {
    ...
    docker.getContainer(name).inspect((err, container) => {
      ...
      findLabeledContainer(docker, linkLabels, name)
        .then((container) => {
          if (container) {
        ...
        .catch((err) => {
          ...
          return reject(new Error(`No container found for link: ${link}`));
        });
    });
  });
}
